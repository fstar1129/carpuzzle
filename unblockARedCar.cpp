...
//		Step(2, MovementDirection.Up),
		for(int i = 1; i <= brickCount; i++) {
			...
			if(tempV.size() > 0 && mp[tempV] <= 0) {
				...
				mp[tempV] = 1;
				tempC->move = "Step(" + to_string(i) + ", MovementDirection.Down)";
			}
			tempV = moveUp(currBoard->board,i);
			if(tempV.size() > 0 && mp[tempV] <= 0) {
				tempC = new config; tempC->board = tempV; tempC->parent = currBoard; que.push(tempC);
				 mp[tempV] = 1; tempC->move = "Step(" + to_string(i) + ", MovementDirection.Up)";
			}
			tempV = moveRight(currBoard->board,i);
			if(tempV.size() > 0 && mp[tempV] <= 0) {
				tempC = new config; tempC->board = tempV; tempC->parent = currBoard; que.push(tempC);
				mp[tempV] = 1; tempC->move = "Step(" + to_string(i) + ", MovementDirection.Right)";
			}
			tempV = moveLeft(currBoard->board,i);
			if(tempV.size() > 0 && mp[tempV] <= 0) {
				tempC = new config; tempC->board = tempV; tempC->parent = currBoard; que.push(tempC);
				mp[tempV] = 1; tempC->move ="Step(" + to_string(i) + ", MovementDirection.Left)";
			}
		}
		delete temp;
	}
